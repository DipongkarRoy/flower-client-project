...
const CheekOut = () => {
    ...
    const { title, price, } = flower
   
    if (user !== null && typeof object === 'object' && 'email' in user) {
        // Access the 'email' property
       return user.email;
        // Continue with your code
    } else {
        // Handle the case where the object is null or doesn't have an 'email' property
        console.error("Object is null or doesn't have an 'email' property");
    }
    ...
    return (
        <div className="box">
            <h2 className="text-3xl text-center text-red-400">Book service :{title} </h2>
            <form onSubmit={handleOrderData}>
                ...
                <div className="md:flex gap-4">
                    <div className="form-control w-[50%]">
                        ...
                        <input type="text" name="email"  defaultValue={user.email?user.email : "not accound"}className="input input-bordered" required />
                    </div>
                    <div className="form-control w-[50%]">
                        <label className="label">
                            <span className="label-text">Price</span>
